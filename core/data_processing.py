...
def port_min_variance_daily_return(dataframe, dataframe_test):
    from pypfopt.efficient_frontier import EfficientFrontier
    import pandas as pd
    import numpy as np
    ...
    portfolio = portfolio.T
    portfolio.columns = ['account_value']
    portfolio_daily_return = portfolio.reset_index(inplace=False)
    portfolio_daily_return.rename(columns = {'index': 'date'}, inplace = True)
    portfolio_daily_return['daily_return'] = portfolio_daily_return['account_value'].pct_change(1)
